...
lawn = gr.Polygon(gr.Point(0,400),gr.Point(0,600),
                  gr.Point(600,600), gr.Point(600,400))
lawn.setFill('green')

sky = gr.Polygon(gr.Point(0,0),gr.Point(0,400),
                 gr.Point(600,400),gr.Point(600,0))
sky.setFill('blue')


def draw_spartak(x):            #Лого Спартака и табличку под ним
...
lineright = gr.Line(gr.Point(450, 400),gr.Point(520, 600)) #Рисует разметку поля
lineright.setWidth(15)
lineright.setOutline('white')

linerleft = gr.Line(gr.Point(150, 400),gr.Point(80, 600))  #Рисует разметку поля
linerleft.setWidth(15)
linerleft.setOutline('white')

gatebar_up = gr.Line(gr.Point(195, 300),gr.Point(405, 300)) #Верхняя штанга
gatebar_up.setWidth(10)
gatebar_up.setOutline('gray')

linerup = gr.Line(gr.Point(145, 400),gr.Point(455, 400))   #Рисует разметку поля
linerup.setWidth(5)
...
bump = gr.Polygon(gr.Point(180,320),gr.Point(220,320),
                  gr.Point(230,360), gr.Point(200,375),
                   gr.Point(170,360))         #Рисует мишень для мяча на штанге
bump.setFill('red')
...
while True==True:                  #Бесконечный цикл, в котором мяч....
    for i in range (100):          #...перемещается с поля в мишень...
        time.sleep(0.01)
        ball.move(-1.5,-1.5)
    for i in range (100):          #...и обратно
        time.sleep(0.01)
        ball.move(1.5,1.5)
